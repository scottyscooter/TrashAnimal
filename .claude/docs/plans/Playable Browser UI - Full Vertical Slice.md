...
**Review notes (added after plan review — all 8 accepted):**
1. **Locking discipline was under-specified.** `GameSessionEntry` mutates `Revision` only via a private-setter + `IncrementRevision()` method, and all mutation happens while `GameApplicationService.WithSessionLockAsync` holds `entry.Lock`. `LobbyEntry` must follow the same discipline — private setters, mutation only via methods, and `JoinLobbyAsync`/`StartLobbyAsync` must run their entire read-check-mutate sequence inside `entry.Lock`. Without this: concurrent joins can both pass the "seat < 4" or "nickname unique" checks and corrupt state; a double-click on Start can call `GameApplicationService.CreateGameAsync` twice; a join can race a start (TOCTOU) and leave a player believing they joined a lobby that already started without them.
2. **The tuple→record change is not confined to `TokenPhaseView.cs`.** Verified usage: `IPlayerController.ChooseDoubleStashCardIds`/`ChooseStashTrashStashCard` and their implementations in `CliHumanController.cs`/`AiController.cs`, plus a call site in `Program.cs:230` (`Array.Empty<(Guid, CardName)>()`), all type the parameter as the raw tuple `(Guid Id, CardName Name)`. A named record is not implicitly convertible from a tuple, so this change touches those 4 files in `TrashAnimal/` (domain project) too, and requires re-running `TrashAnimal.Tests`, not just the new API integration tests. Track as its own subtask with its own file list, not a one-line aside.
3. **Validation ordering:** `GameApplicationService.CreateGameAsync` throws `ArgumentException` if player count isn't 2–4. `LobbyApplicationService.StartLobbyAsync` must run its own 2–4 seat check and return the intended 422 *before* calling into `CreateGameAsync` — otherwise an uncaught `ArgumentException` from that (untouched) method surfaces as a 500.
4. **No nickname input validation was specified.** `CreateLobbyRequest`/`JoinLobbyRequest` need an emptiness/whitespace/max-length check on `Nickname`, at the controller layer, consistent with how `GamesController.CreateGame` validates `PlayerNames` count.
5. **No leave/seat-release mechanism.** `LobbyHub.LeaveLobbyAsync` only drops SignalR group membership, not the seat itself. There's no `LobbyApplicationService` method to release a seat if a player joins and abandons the tab pre-start, which can permanently deadlock a lobby at 3/4. Decide explicitly: add a minimal seat-release path, or document it as an accepted MVP limitation.
6. **No lobby eviction/cleanup.** `InMemoryLobbyRepository` (mirroring `InMemoryGameSessionRepository`) never removes entries. This is an existing accepted limitation for game sessions, but lobbies are more likely to be abandoned pre-start (created and never shared) — confirm this is a conscious carry-forward, not an oversight.
7. **`ClientToken` generation was unspecified.** State explicitly that it must be an unguessable value (e.g. `Guid.NewGuid().ToString()`) since it's the sole authorization for the admin-only Start action and the reconnection mechanism.
8. **Test coverage gaps:** `LobbyFlowTests.cs` needs cases beyond the happy/rejection paths — concurrent joins racing the 4-seat limit, join-during-start race, and an explicit assertion that lobby seat index → game `playerSeat` mapping produces the right hand for the right nickname (easy to get silently wrong since `CreateGameAsync` re-derives players from name order, not seat index directly).

- Subtask: Implement `LobbyEntry`/`LobbySeat`/`ILobbyRepository`/`InMemoryLobbyRepository` mirroring `GameSessionEntry`/`IGameSessionRepository` patterns, including `GameSessionEntry`'s private-setter/method-mutation discipline for `LobbyEntry`'s `Revision`/`IsStarted`/`GameId` (review note 1).
- Subtask: Implement `LobbyApplicationService` (create/join/get/start). `JoinLobbyAsync` and `StartLobbyAsync` must hold `entry.Lock` for their full read-check-mutate sequence (review note 1). `StartLobbyAsync` must validate the 2–4 seat range itself and return 422 before ever calling `GameApplicationService.CreateGameAsync` (review note 3). Generate `ClientToken` as an unguessable value, e.g. `Guid.NewGuid().ToString()` (review note 7).
- Subtask: Add Lobby request/response contract records under `Contracts/Requests` and `Contracts/Responses`, with nickname validation (non-empty after trim, max length) enforced at the controller layer on `CreateLobbyRequest`/`JoinLobbyRequest` (review note 4).
- Subtask: Implement `LobbiesController` with the 4 routes and status codes described in section A.
- Subtask: Implement `LobbyHub` + `ILobbyUpdatePublisher`/`SignalRLobbyUpdatePublisher`/`StubLobbyUpdatePublisher`, mount at `/hubs/lobby`. Decide and implement seat handling on `LeaveLobbyAsync` — either release the seat or explicitly document the pre-start-abandonment deadlock as an accepted MVP limitation (review note 5).
- Subtask: Confirm/document that `InMemoryLobbyRepository` has no eviction, same as `InMemoryGameSessionRepository`, as a conscious carry-forward rather than an oversight (review note 6).
- Subtask: Change `TokenPhaseView.StashableHandCardsForCurrentPrompt` from a `(Guid, CardName)` tuple to a named `StashableHandCard(Guid CardId, CardName Name)` record. Update all dependent signatures: `IPlayerController.ChooseDoubleStashCardIds`/`ChooseStashTrashStashCard`, their implementations in `CliHumanController.cs` and `AiController.cs`, and the `Array.Empty<(Guid, CardName)>()` call site in `Program.cs:230` (review note 2). Re-run `TrashAnimal.Tests` in full, not just the new API integration tests.
- Subtask: Write `TrashAnimal.Api.Tests/Integration/LobbyFlowTests.cs` covering create → join → duplicate-nickname rejection → full-lobby rejection → start (wrong token rejected, then correct) → resulting `GameId` is playable, plus concurrent-join-at-4-seats, join-during-start race, and seat-index-to-playerSeat mapping correctness (review note 8). Tag also `test`.
