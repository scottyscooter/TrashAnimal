...
/// stock of cards. Used by tests that need a deterministic deck size/composition without
/// depending on the real <see cref="Deck"/>'s shuffling.
/// </summary>
internal static class DrawPileMockFactory
{
    ...
    internal static Mock<IDrawPile> CreateEmpty()
    {
        var drawPileMock = new Mock<IDrawPile>();
        drawPileMock.Setup(pile => pile.GetDeckCount()).Returns(0);
        drawPileMock.Setup(pile => pile.DealCards(It.IsAny<int>())).Returns(Array.Empty<Card>());
        return drawPileMock;
    }
}
