...
function DiscardCarouselModal({ discardPile, onClose }: DiscardCarouselModalProps) {
  ...
  if (total === 0) {
    return (
      <Modal onClose={onClose} labelledBy="discard-carousel-heading">
        <h2 id="discard-carousel-heading" className="text-lg font-semibold" style={{ color: 'var(--gb-text-primary)' }}>
          DISCARD PILE
        </h2>
        <p className="mt-2" style={{ color: 'var(--gb-text-label)' }}>
          Nothing here yet.
        </p>
      </Modal>
    );
  ...
  return (
    <Modal onClose={onClose} labelledBy="discard-carousel-heading" wide>
      <h2 id="discard-carousel-heading" className="mb-4 text-lg font-semibold" style={{ color: 'var(--gb-text-primary)' }}>
        DISCARD PILE
      </h2>
      <div className="flex items-center justify-center gap-4">
        <button
          ...
          className="gb-glass flex h-[52px] w-[52px] items-center justify-center rounded-full text-xl disabled:opacity-30"
        ...
        <div className="flex items-center gap-4">
          {prev && (
            <img
              ...
              alt={prev.name}
              className="h-[238px] w-[170px] rounded-lg object-cover opacity-55 transition-all duration-[250ms]"
          ...
          <img
            ...
            alt={current.name}
            className="h-[364px] w-[260px] rounded-lg object-cover shadow-2xl transition-all duration-[250ms]"
          />
          {next && (
            <img
      ...
      <p className="mt-4 text-center text-sm" style={{ color: 'var(--gb-text-label)' }}>
        {index + 1} / {total}
      </p>
    </Modal>
  );
